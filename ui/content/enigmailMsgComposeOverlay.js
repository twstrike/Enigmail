...
Enigmail.msg = {
  ...
  doPgpButton: function (what)
  {
    ...
    try {
      if (!this.getAccDefault("enabled")) {
        if (Dialog.confirmDlg(window, EnigmailLocale.getString("configureNow"),
  ...
  determineSendFlags: function ()
  {
    ...
    if (this.getAccDefault("enabled")) {
      ...
      if (toAddrList.length > 0 && this.encryptByRules == EnigmailConstants.ENIG_UNDEF && Prefs.getPref("autoSendEncrypted") == 1) {
...
*/
  ...
  signingNoLongerDependsOnEnc: function ()
  {
    if (this.finalSignDependsOnEncrypt) {
      ...
      Dialog.alertPref(window, EnigmailLocale.getString("signIconClicked"), "displaySignWarn");
    }
  },
  ...
  processRules: function (forceRecipientSettings, sendFlags, optSendFlags, toAddrStr, bccAddrStr)
  {
    ...
    if (toAddrStr.length > 0 && this.encryptByRules == EnigmailConstants.ENIG_UNDEF && Prefs.getPref("autoSendEncrypted") == 1) {
  ...
  encryptTestMessage: function (enigmailSvc, sendFlags, optSendFlags, fromAddr, toAddrStr, bccAddrStr, bccAddrList, refresh)
  {
    ...
    if (testStatusFlagsObj.value) {
      ...
      if (testErrorMsgObj.value.search(s) >= 0)  {
        Dialog.alert(window, EnigmailLocale.getString("errorKeyUnusable", [ fromAddr ]));
        return null;
      }
    }
  ...
  encryptMsg: function (msgSendType)
  {
    ...
    if (gWindowLocked) {
      Dialog.alert(window, EnigmailLocale.getString("windowLocked"));
      return false;
    }
    ...
    if (!enigmailSvc) {
       ...
       return Dialog.confirmDlg(window, msg, EnigmailLocale.getString("msgCompose.button.send"));
    }
    ...
    try {
      ...
      if ((sendFlags&ENCRYPT) === 0 &&
       ...
       return Dialog.confirmDlg(window, msg, EnigmailLocale.getString("msgCompose.button.sendUnencrypted"));
    }
        ...
        if (sendInfo.sendFlags & (ENCRYPT | SIGN)) {
          ...
          if (errorMsgObj.value) {
            ...
            if (errorMsgObj.value.search(s) >= 0)  {
              Dialog.alert(window, EnigmailLocale.getString("errorKeyUnusable", [ sendInfo.fromAddr ]));
              return false;
            }
          }
  ...
  sendAborted: function (window, errorMsgObj)
  {
    if (errorMsgObj && errorMsgObj.value) {
      ...
      Dialog.alert(window, EnigmailLocale.getString("sendAborted") + txt);
    }
    else {
      Dialog.alert(window, EnigmailLocale.getString("sendAborted") + "an internal error has occurred");
    }
  },
