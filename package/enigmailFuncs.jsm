...
const EnigmailFuncs = {
  ...
  stripEmail: function (mailAddrs)
  {
    ...
    while ((qStart = mailAddrs.indexOf('"')) != -1) {
       ...
       if (qEnd == -1) {
         EnigmailLog.ERROR("enigmailFuncs.jsm: stripEmail: Unmatched quote in mail address: "+mailAddrs+"\n");
  ...
  formatPlaintextMsg: function (plainTxt)
  {
    ...
    //EnigmailLog.DEBUG("enigmailFuncs.jsm: r='"+r+"'\n");
    return r;
  },
  ...
  getHeaderData: function (data) {
    EnigmailLog.DEBUG("enigmailFuncs.jsm: getHeaderData: "+data.substr(0, 100)+"\n");
    var a = data.split(/\n/);
    var res = [];
    for (let i = 0; i < a.length; i++) {
      ...
      for (let j=0; j < b.length; j++) {
        ...
        if (m) {
          ...
          EnigmailLog.DEBUG("enigmailFuncs.jsm: getHeaderData: "+m[2].toLowerCase()+" = "+res[m[2].toLowerCase()] +"\n");
        }
      }
